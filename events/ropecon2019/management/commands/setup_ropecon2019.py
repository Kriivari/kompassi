...
class Setup(object):
    ...
    def setup_labour(self):
        ...
        labour_event_meta_defaults = dict(
            ...
            work_begins=datetime(2019, 7, 25, 12, 0, tzinfo=self.tz),
            work_ends=datetime(2019, 7, 30, 12, 0, tzinfo=self.tz),
        ...
        AlternativeSignupForm.objects.get_or_create(
            ...
            defaults=dict(
                ...
                active_from=datetime(2018, 10, 1, 12, 0, 0, tzinfo=self.tz),
                active_until=datetime(2019, 7, 20, 23, 59, 59, tzinfo=self.tz),
            ),
        )
    ...
    def setup_programme(self):
        ...
        for start_time, end_time in [
            (
                datetime(2019, 7, 26, 15, 0, tzinfo=self.tz),
                datetime(2019, 7, 28, 18, 0, tzinfo=self.tz),
            ),
        ]:
        ...
        SpecialStartTime.objects.get_or_create(
            event=self.event,
            start_time=datetime(2019, 7, 26, 12, 0, tzinfo=self.tz),
        )
...
Pelinjohtajat saavat Ropeconin viikonloppurannekkeen kahdeksan tunnin pelautuksella tai päivärannekkeen neljän tunnin pelautuksella. Lisäksi pelinjohtajat palkitaan sunnuntaina jaettavalla lootilla, eli ilmaisella roolipelitavaralla. Mitä useamman pelin pidät, sitä korkeammalle kohoat loottiasteikossa!
                ...
                active_from=datetime(2019, 4, 1, 0, 0, tzinfo=self.tz),
                active_until=datetime(2019, 7, 1, 23, 59, 59, tzinfo=self.tz),
                num_extra_invites=0,
                order=20,
            )
        )
...
Ohjelman lisäksi haemme työvoimaa lautapelitiskille, joka huolehtii pelien lainaamisesta ja kunnossa pysymisestä. Ilmoittaudu lautapelitiskin työntekijäksi täyttämällä työvoimalomake.
                ...
                active_from=datetime(2019, 4, 1, 0, 0, tzinfo=self.tz),
                active_until=datetime(2019, 5, 31, 23, 59, 59, tzinfo=self.tz),
                num_extra_invites=0,
                order=60,
            )
        )
